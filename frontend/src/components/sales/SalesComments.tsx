                ...
                <List dataSource={comments} renderItem={(item: any) => (
                    <List.Item>
                        <List.Item.Meta
                            ...
                            description={<div><div style={{ color: '#333' }}>{item.content}</div>{item.sender_type === 'CUSTOMER' && <Button type="text" size="small" icon={item.is_visible ? <EyeOutlined /> : <EyeInvisibleOutlined />} onClick={() => toggle(item.id)}>{item.is_visible ? 'Hiện' : 'Ẩn'}</Button>}</div>}
                        />
                    </List.Item>
                )} />
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
                <Input value={text} onChange={e => setText(e.target.value)} placeholder="Nhập tin nhắn..." onPressEnter={send} />

  ...
  return (
    <div style={{ padding: 40, background: '#fff', fontSize: 14, fontFamily: 'Times New Roman, serif', color: '#000' }}>
      ...
      <Row justify="space-between" align="middle" style={{borderBottom: '2px solid #1890ff', paddingBottom: 20, marginBottom: 30}}>
          <Col span={12}><div style={{border: '2px solid #1890ff', padding: '10px 20px', display: 'inline-block', textAlign:'center', minWidth: 200}}><Title level={4} style={{margin: 0, color: '#1890ff', textTransform: 'uppercase'}}>HULA</Title><Text type="secondary">NỆM MẦM NON</Text></div></Col>
          <Col span={12} style={{textAlign: 'right'}}><Title level={2} style={{margin: 0, color: '#1890ff', textTransform: 'uppercase'}}>{docTitle}</Title><div>Số: <b>{data.order_code}</b></div><div>TP. HCM, ngày {dayjs(data.order_date).format('DD')} tháng {dayjs(data.order_date).format('MM')} năm {dayjs(data.order_date).format('YYYY')}</div></Col>
      </Row>
      ...
      <Table
        dataSource={data.items} pagination={false} rowKey="id" bordered size="small"
        columns={[
            { title: 'STT', width: 50, align: 'center', render: (_:any, __:any, index:number) => index + 1 },
            { title: 'Tên Sản phẩm', render: (r:any) => (<div><div style={{fontWeight:600}}>{r.sku}</div>{r.is_sample_approved && <div style={{marginTop: 5, fontSize: 12}}><Tag color="success" icon={<CheckCircleFilled />}>Mẫu đã duyệt</Tag></div>}{r.variant_color && <div style={{fontSize:12, color:'#666'}}>Màu: {r.variant_color}</div>}</div>) },
            { title: 'Mô tả', render: (r:any) => <div style={{fontSize: 12, color: '#555'}}>{r.sample_note}</div> },
        ...
        summary={() => (
            <>
                <Table.Summary.Row><Table.Summary.Cell index={0} colSpan={6} align="right"><b>TỔNG CỘNG:</b></Table.Summary.Cell><Table.Summary.Cell index={1} align="right"><b style={{fontSize: 16, color: '#cf1322'}}>{Number(data.total_amount).toLocaleString()} ₫</b></Table.Summary.Cell></Table.Summary.Row>
                {paidAmount > 0 && <Table.Summary.Row><Table.Summary.Cell index={0} colSpan={6} align="right"><b style={{color: 'green'}}>ĐÃ CỌC:</b></Table.Summary.Cell><Table.Summary.Cell index={1} align="right"><b style={{color: 'green'}}>{paidAmount.toLocaleString()} ₫</b></Table.Summary.Cell></Table.Summary.Row>}
            </>
        )}
      />
      ...
      <Row style={{marginTop: 50, textAlign: 'center'}}><Col span={12}><b>ĐẠI DIỆN KHÁCH HÀNG</b><br/><i>(Ký, ghi rõ họ tên)</i></Col><Col span={12}><b>ĐẠI DIỆN CÔNG TY TƯỜNG LINH</b><br/><i>(Ký, đóng dấu)</i><div style={{height: 80}}></div></Col></Row>
    </div>
  );
};

...
const HeaderNotifications: React.FC = () => {
    ...
    // Giả lập user ID (thực tế lấy từ context/localStorage)
    ...
    const fetchNoti = async () => {
        if (!userId) return;
        try {
            const res = await axios.get(`${API_URL}/notifications?user_id=${userId}`);
    ...
    useEffect(() => {
        fetchNoti();
        const interval = setInterval(fetchNoti, 30000); // Polling mỗi 30s
        return () => clearInterval(interval);
    }, [userId]);

    const handleRead = async (item: any) => {
        if(!item.is_read) {
            await axios.post(`${API_URL}/notifications/${item.id}/read`);
            fetchNoti();
        }
    ...
    const handleReadAll = async () => {
        await axios.post(`${API_URL}/notifications/read-all`, { user_id: userId });
        fetchNoti();
    };

...
import { Modal, Form, Input, Select, InputNumber, Row, Col, Tabs, Divider, Button, message, Table, DatePicker, Typography, Statistic, Space } from 'antd';
...
const SalesOrderDetail: React.FC<Props> = ({ open, onClose, onSuccess, initialData, isQuotation, customers, products }) => {
    ...
    const handleSave = async (values: any) => {
        try {
            ...
            if (initialData && initialData.id) await axios.put(`${API_URL}/sales/quote/${initialData.id}`, payload);
    ...
    const handleProductChange = (val: string, index: number) => {
        ...
        if (p) {
            ...
            currentItems[index].unit = p.unit; // Set unit for display (need to handle in render)
    ...
    return (
        <Modal
            title={isQuotation ? "Báo Giá Chi Tiết" : "Quản Lý Đơn Hàng"}
        ...
        >
            <Form form={form} layout="vertical" onFinish={handleSave}>
                ...
                <Tabs defaultActiveKey="1" items={[
                        ...
                        children: (
                            <Row gutter={24}>
                                <Col span={15} style={{ borderRight: '1px solid #f0f0f0' }}>
                                    <Row gutter={16}>
                                        <Col span={12}><Form.Item name="customer_id" label={<span style={{color:'red'}}>* Khách Hàng</span>} rules={[{required:true}]}><Select showSearch optionFilterProp="label" options={customers.map(c => ({ label: `${c.code} - ${c.name}`, value: c.id }))} onChange={handleCustomerChange} /></Form.Item></Col>
                                        <Col span={12}><Form.Item name="order_code" label="Mã Đơn"><Input disabled style={{ fontWeight: 'bold', color: '#1890ff' }} /></Form.Item></Col>
                                    </Row>
                                    
                                    {/* --- VUNG NHAP LIEU SAN PHAM (TABLE STYLE) --- */}
                                    <div style={{background: '#fafafa', padding: 10, borderRadius: 6, border: '1px solid #eee'}}>
                                        <Row gutter={8} style={{marginBottom: 5, fontWeight: 600, color: '#666', fontSize: 13}}>
                                        ...
                                        <Form.List name="items">
                                            {(fields, { add, remove }) => (
                                                <>
                                                            ...
                                                            <Row key={key} gutter={8} style={{ marginBottom: 8 }} align="middle">
                                                                <Col span={9}>
                                                                    <Form.Item {...restField} name={[name, 'sku']} style={{ marginBottom: 0 }} rules={[{ required: true }]}>
                                                                        <Select placeholder="Chọn SP..." options={products} onChange={(v) => handleProductChange(v, name)} dropdownMatchSelectWidth={400} />
                                                                    </Form.Item>
                                                                </Col>
                                                                <Col span={3}>
                                                                ...
                                                                <Col span={3} style={{textAlign:'center'}}>
                                                                    <span style={{color:'#888', fontSize:12}}>{currentProd?.unit || '-'}</span>
                                                                </Col>
                                                                <Col span={4}>
                                                                ...
                                                                <Col span={4} style={{textAlign:'right', fontWeight:500}}>
                                                    ...
                                                    <Button type="dashed" onClick={() => add()} block icon={<PlusOutlined />} style={{marginTop:5}}>Thêm dòng</Button>
                                                </>
                                            )}
                                        </Form.List>
                                        ...
                                        {/* --- TOTALS SECTION --- */}
                                        <Row justify="end" style={{textAlign: 'right', lineHeight: '2em'}}>
                                            <Col span={12}>
                                                <Row>
                                                ...
                                                <Row align="middle">
                                                    ...
                                                    <Col span={10}>
                                                        ...
                                                        <span style={{marginLeft: 10}}>{vatAmount.toLocaleString()}</span>
                                                    </Col>
                                                </Row>
                                                <Row>
                                                ...
                                                <Row>
                                                    ...
                                                    <Col span={10}><Text strong style={{fontSize: 16, color: '#1890ff'}}>{totalAmount.toLocaleString()} ₫</Text></Col>
                                                </Row>
                                            </Col>
                                        </Row>
                                    </div>
                    ...
                    !isQuotation && { key: '2', label: 'Thanh toán & Xuất kho', children: <div>(Vui lòng lưu đơn hàng để thao tác)</div> }

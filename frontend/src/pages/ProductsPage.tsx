...
const ProductsPage: React.FC = () => {
  ...
  const fetchData = async () => {
    ...
    try { 
      ...
      setMaterials(resMat.data.map((m:any) => ({ label: `${m.name} (${m.code})`, value: m.id, unit: m.unit, price: Number(m.cost_per_unit) || 0 })));
  ...
  useEffect(() => {
      ...
      filtered.forEach((p: any) => {
          ...
          if (!groups[groupKey]) {
              groups[groupKey] = { 
                  key: 'group_' + groupKey, isGroup: true, name: p.name, category: catName,
                  product_type: p.product_type, base_price: p.base_price, children: [], totalStock: 0, minPrice: Infinity, maxPrice: -Infinity 
              };
          }
      ...
      setTreeData(Object.values(groups).map((g: any) => {
  ...
  const calcBomTotal = () => (bomValues || []).reduce((sum:number, item:any) => {
      ...
      return sum + ((Number(item.quantity)||0) * (mat ? mat.price : 0) * (1 + (Number(item.waste_percent)||0)/100));
  }, 0);
  ...
  const handleAddVariant = (group: any) => {
      ...
      form.setFieldsValue({ name: group.name, category: group.category, product_type: group.product_type, base_price: group.base_price, profit_margin: 30 });
      setIsModalOpen(true);
  };
  const handleSaveProduct = async (values: any) => {
    try {
      const payload = { ...values, attributes: { color: values.color, size: values.size, fabric: values.fabric }, base_price: Number(values.base_price)||0, quantity_in_stock: Number(values.quantity_in_stock)||0 };
  ...
  const handleSaveAll = async () => {
      ...
      try {
          ...
          await axios.put(`${API_URL}/products/${editingItem.id}`, { ...form.getFieldsValue(), base_price: suggestedPrice, cost_price: newCost });
      ...
      } catch(e) { message.error('Lỗi lưu chi tiết'); }
  };
  const handleSyncVariants = async (item: any) => {
      const targetItem = item || editingItem;
      if(!targetItem) return;
      Modal.confirm({
          title: 'Đồng bộ?', content: 'Sao chép cấu hình sang các biến thể khác?',
  ...
  const handleRoutingChange = async (index: number, field: string, value: any) => {
      ...
      if (supplierId && stepName && editingItem?.id) {
          ...
          if (process) {
              try {
                  const res = await axios.post(`${API_URL}/suppliers/check-price`, { supplierId, processId: process.id, productId: editingItem.id });
                  if (res.data.price > 0) {
  ...
  const openEditModal = (r: any) => {
      setEditingItem(r);
      form.setFieldsValue({ ...r, color: r.attributes?.color, size: r.attributes?.size, fabric: r.attributes?.fabric, profit_margin: r.profit_margin || 30 });
      setTimeout(async () => {
          try {
             const resBom = await axios.get(`${API_URL}/products/${r.sku}/boms`); form.setFieldValue('boms', resBom.data);
             const resRoute = await axios.get(`${API_URL}/products/${r.id}/routings`);
             let routeData = resRoute.data;
             if(!routeData || routeData.length === 0) routeData = [{ step_name: 'Cắt', is_required: true, cost: 0 }, { step_name: 'May', is_required: true, cost: 0 }];
             form.setFieldValue('routings', routeData);
             const resLog = await axios.get(`${API_URL}/products/${r.id}/logistics`); form.setFieldValue('logistics', resLog.data);
          } catch(e) {}
      }, 200);
      setIsModalOpen(true);
  };
  ...
  const MaterialInfoRow = ({ matId, qty, waste }: any) => {
      ...
      return <Row gutter={4} style={{fontSize:12, marginTop:5, color:'#666'}}><Col span={10}><Tag>{mat.unit}</Tag> {mat.name}</Col><Col span={14} style={{textAlign:'right', fontWeight:'bold', color:'#1890ff'}}>= {subTotal.toLocaleString(undefined, {maximumFractionDigits:0})} đ</Col></Row>;
  };
  ...
  return (
    <div>
      ...
      <Modal title={editingItem ? `Chi tiết: ${editingItem.sku}` : "Thêm Sản Phẩm Mới"} open={isModalOpen} onCancel={() => setIsModalOpen(false)} footer={null} width={900} style={{top: 20}}>
        <Form form={form} layout="vertical" onFinish={handleSaveProduct} initialValues={{profit_margin: 30}}>{/* ...Content... */}
          <Tabs defaultActiveKey="1" items={[
                      ...
                      <>
                        ...
                        <div style={{background: '#f6ffed', padding: '15px', borderRadius: 8, border: '1px solid #b7eb8f', marginTop: 10}}>
                            <Row gutter={16} align="middle">
                                ...
                                <Col span={6}><Form.Item name="profit_margin" label="% Lợi Nhuận" style={{marginBottom:0}}><InputNumber min={0} max={500} style={{width:'100%'}} suffix="%" /></Form.Item></Col>
                                <Col span={1} style={{textAlign:'center'}}><ArrowRightOutlined /></Col>
                                <Col span={8}><Form.Item name="base_price" label="Giá Bán (Đề xuất)" style={{marginBottom:0}} help={`Gợi ý: ${suggestedPrice.toLocaleString()} đ`}><InputNumber style={{width:'100%', fontWeight:'bold', color:'green'}} formatter={v => `${v}`.replace(/\B(?=(\d{3})+(?!\d))/g, ',')} addonAfter="₫" /></Form.Item></Col>
                            </Row>
                            <div style={{textAlign:'right', marginTop:5}}><Button type="link" size="small" onClick={() => form.setFieldValue('base_price', suggestedPrice)}><CalculatorOutlined /> Áp dụng giá gợi ý</Button></div>
                        </div>
              ...
              },
              {
                  ...
                  children: (
                      <Form.List name="boms">{(fields,{add,remove})=>(<div style={{maxHeight: 350, overflowY: 'auto'}}>{fields.map(({key,name,...restField})=>(<div key={key} style={{marginBottom:8,borderBottom:'1px dashed #f0f0f0',paddingBottom:5}}><Row gutter={8} align="top"><Col span={10}><Form.Item {...restField} name={[name,'material_id']} noStyle rules={[{required:true}]}><Select placeholder="Chọn NPL..." showSearch optionFilterProp="label" options={materials} style={{width:'100%'}} /></Form.Item></Col><Col span={4}><Form.Item {...restField} name={[name,'quantity']} noStyle><InputNumber placeholder="SL" style={{width:'100%'}} min={0} /></Form.Item></Col><Col span={3}><Form.Item {...restField} name={[name,'waste_percent']} noStyle><InputNumber placeholder="%" style={{width:'100%'}} min={0} /></Form.Item></Col><Col span={6}><Form.Item shouldUpdate>{()=><MaterialInfoRow matId={form.getFieldValue(['boms',name,'material_id'])} qty={form.getFieldValue(['boms',name,'quantity'])} waste={form.getFieldValue(['boms',name,'waste_percent'])} />}</Form.Item></Col><Col span={1}><MinusCircleOutlined onClick={()=>remove(name)} style={{color:'red',marginTop:5}} /></Col></Row></div>))}<Button type="dashed" onClick={()=>add()} block icon={<PlusOutlined/>}>Thêm nguyên liệu</Button><div style={{textAlign:'right',marginTop:10}}>Tổng NPL: <b>{calcBomTotal().toLocaleString()} đ</b></div></div>)}</Form.List>
                  )
              },
              {
                  ...
                  children: (
                      <div style={{maxHeight: 450, overflowY: 'auto'}}>
                        <div style={{background:'#fafafa', padding: 8, borderBottom:'1px solid #eee', fontWeight:'bold', marginBottom:10}}><Row gutter={8}><Col span={1}></Col><Col span={11}>Công Đoạn (Quy trình)</Col><Col span={7}>Nhà Gia Công</Col><Col span={4}>Đơn Giá</Col><Col span={1}></Col></Row></div>
                        <Form.List name="routings">{(fields,{add,remove})=>(<div>{fields.map(({key,name,...restField})=>(<Row key={key} gutter={8} align="middle" style={{marginBottom:8,background:'#f9f9f9',padding:8,borderRadius:4}}><Col span={1}><Form.Item {...restField} name={[name,'is_required']} valuePropName="checked" noStyle><Checkbox/></Form.Item></Col><Col span={11}><Form.Item {...restField} name={[name,'step_name']} noStyle rules={[{required:true}]}><Select placeholder="Công đoạn..." showSearch optionFilterProp="label" options={processes.map(p=>({label:p.label,value:p.label,id:p.id}))} onChange={(val)=>handleRoutingChange(key,'step',val)} /></Form.Item></Col><Col span={7}><Form.Item {...restField} name={[name,'supplier_id']} noStyle><Select placeholder="Nhà Gia Công..." options={suppliers} allowClear onChange={(val)=>handleRoutingChange(key,'supplier',val)} /></Form.Item></Col><Col span={4}><Form.Item {...restField} name={[name,'cost']} noStyle><InputNumber placeholder="Giá" style={{width:'100%'}} formatter={v=>`${v}`.replace(/\B(?=(\d{3})+(?!\d))/g,',')} /></Form.Item></Col><Col span={1}><DeleteOutlined onClick={()=>remove(name)} style={{color:'red',cursor:'pointer'}} /></Col></Row>))}<Button type="dashed" onClick={()=>add()} block icon={<PlusOutlined/>}>Thêm công đoạn</Button></div>)}</Form.List>
                        ...
                        <Form.List name="logistics">{(fields,{add,remove})=>(<>{fields.map(({key,name,...restField})=>(<Row key={key} gutter={8} style={{marginBottom:8}}><Col span={12}><Form.Item {...restField} name={[name,'route_name']} noStyle><Input placeholder="Tên chặng..." /></Form.Item></Col><Col span={10}><Form.Item {...restField} name={[name,'cost']} noStyle><InputNumber placeholder="Chi phí" style={{width:'100%'}} /></Form.Item></Col><Col span={2}><DeleteOutlined onClick={()=>remove(name)} style={{color:'red'}} /></Col></Row>))}<Button type="dashed" onClick={()=>add()} block icon={<PlusOutlined/>}>Thêm chặng vận chuyển</Button></>)}</Form.List>
        ...
        {editingItem && (<div style={{marginTop: 20, paddingTop: 15, borderTop: '2px solid #eee', background:'#fff', position:'sticky', bottom:0}}><Row align="middle" justify="space-between"><Col><Space><Button type="default" icon={<CopyOutlined />} onClick={() => handleSyncVariants(editingItem)}>Đồng bộ biến thể</Button><Statistic title="Tổng Giá Vốn" value={totalCostEstimate} valueStyle={{color: '#cf1322', fontSize: 18, fontWeight: 'bold'}} prefix="~" suffix="₫" /></Space></Col><Col><Button type="primary" size="large" icon={<SaveOutlined />} onClick={handleSaveAll}>LƯU & CẬP NHẬT GIÁ</Button></Col></Row></div>)}
      </Modal>

      <Drawer title={`Combo: ${currentComboSku}`} placement="right" width={400} onClose={() => setComboDrawerOpen(false)} open={comboDrawerOpen}>
          <Space.Compact style={{ width: '100%', marginBottom: 16 }}><Input placeholder="Mã SKU con" value={childSku} onChange={e => setChildSku(e.target.value)} /><InputNumber min={1} value={childQty} onChange={(v:any) => setChildQty(v)} /><Button type="primary" onClick={addComboItem}>+</Button></Space.Compact>

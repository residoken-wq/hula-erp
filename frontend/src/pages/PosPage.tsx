...
const PosPage: React.FC = () => {
    ...
    return (
        <Layout style={{ height: 'calc(100vh - 64px)', overflow: 'hidden' }}>
            ...
            <Content style={{ padding: '16px', overflowY: 'auto', background: '#f0f2f5' }}>
                ...
                <Row gutter={[16, 16]}>
                    {filteredProducts.map(product => {
                        const isOutOfStock = product.quantity_in_stock < 0; // Rule: Must be >= 0
                        return (
                            <Col xs={24} sm={12} md={8} lg={6} xl={4} key={product.id}>
                                <Card
                                    ...
                                    cover={
                                        <div style={{ height: 140, background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', borderBottom: '1px solid #f0f0f0' }}>
                                            {product.image_url ? (
                                ...
                                >
                                    <div style={{ flex: 1 }}>
                                        ...
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: 8 }}>
                                            ...
                                            <Tag color={isOutOfStock ? 'red' : 'green'}>{isOutOfStock ? 'Hết' : 'Kho: ' + product.quantity_in_stock}</Tag>
                                        </div>
                                    </div>
                                    {!isOutOfStock && <Button type="primary" block size="small" style={{ marginTop: 8 }} icon={<PlusOutlined />}>Thêm</Button>}
                                </Card>
                            </Col>
                        )
                    })}
            ...
            {/* RIGHT SIDE: CART */}
            <div style={{ width: 420, background: '#fff', borderLeft: '1px solid #e8e8e8', display: 'flex', flexDirection: 'column', height: '100%' }}>
                {/* CART HEADER */}
                <div style={{ padding: '16px 24px', borderBottom: '1px solid #f0f0f0', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <Button type="text" icon={<DeleteOutlined />} danger onClick={() => setCart([])}>Xóa hết</Button>
                </div>
                ...
                <div style={{ padding: 24, background: '#fafafa', borderTop: '1px solid #e8e8e8' }}>
                    ...
                    <Row gutter={10}>
                        <Col span={12}>
                            <Button block type="dashed" size="large" onClick={() => navigate('/orders')}>Quay lại</Button>
                        </Col>
                        <Col span={12}>

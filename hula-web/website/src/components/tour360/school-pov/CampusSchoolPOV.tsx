...
export function CampusSchoolPOV({ onClose }: CampusSchoolPOVProps) {
    ...
    return createPortal(
        <div
            className="fixed inset-0 z-[9999] w-screen h-screen bg-[#F6F8F5] flex flex-col overflow-hidden select-none font-sans"
            role="dialog"
            aria-modal="true"
            aria-label="Trải nghiệm trường mầm non HULA 3D góc nhìn thứ nhất"
        >
            {/* 1. Top Bar Header (64px desktop / 56px mobile) */}
            <CampusTopHeader
                onClose={onClose}
                activeRole={activeRole}
                currentRoomId={currentRoomId}
                onToggleMap={() => campusWorldState.toggleMap(true)}
                onToggleRoleSelector={() => campusWorldState.toggleRoleSelector(true)}
            />
            ...
            <div className="relative flex-1 w-full h-full min-h-0 min-w-0 overflow-hidden bg-slate-900">
                ...
                {currentRoomId === 'R7' ? (
                    ...
                    <>
                        {/* Left Floating Re-open Inspector Button (when inspector is collapsed) */}
                        {!isInspectorOpen && (
                            <button
                                type="button"
                                onClick={() => campusWorldState.toggleInspector(true)}
                                className="absolute top-4 right-4 z-10 px-4 py-2.5 rounded-2xl bg-white/95 backdrop-blur-md border border-[#DDE5E1] text-[#183B3A] text-xs sm:text-sm font-bold shadow-lg hover:bg-white active:scale-95 flex items-center gap-2"
                        ...
                        <div className="absolute bottom-4 left-3 right-3 sm:left-6 sm:right-auto sm:max-w-md z-10 pointer-events-none flex flex-col gap-2 animate-fadeIn">
